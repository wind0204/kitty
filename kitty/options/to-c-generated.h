...
static void
convert_from_python_cursor_trail_choreographed(PyObject *val, Options *opts) {
    opts->cursor_trail_choreographed = PyObject_IsTrue(val);
}

static void
convert_from_opts_cursor_trail_choreographed(PyObject *py_opts, Options *opts) {
    PyObject *ret = PyObject_GetAttrString(py_opts, "cursor_trail_choreographed");
    if (ret == NULL) return;
    convert_from_python_cursor_trail_choreographed(ret, opts);
    Py_DECREF(ret);
}
...
convert_opts_from_python_opts(PyObject *py_opts, Options *opts) {
    ...
    if (PyErr_Occurred()) return false;
    convert_from_opts_cursor_trail_choreographed(py_opts, opts);
    if (PyErr_Occurred()) return false;
